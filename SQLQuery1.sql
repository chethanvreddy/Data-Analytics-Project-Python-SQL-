...
select top 10 product_id, sum(sales) as sales 
from df_orders
group by product_Id
order by sales desc
...
with cte as (
select category, format(order_date,'yyyyMM') as order_year_month, sum(sales) as sales
from df_orders
...
with cte as (
select sub_category, sum(sales) as sales, year(order_date) as order_year
from df_orders
